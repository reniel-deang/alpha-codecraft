        ...
        @if ($communityPosts->count() > 0)
            @foreach ($communityPosts as $post)
                @if (!$post->reports()->where('reporter_id', Auth::user()->id)->where('community_post_id', $post->id)->first())
                    <div class="max-w-4xl mx-auto mb-10 p-6 antialiased bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <!-- Post Header -->
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    ...
                                    <p class="text-gray-600 dark:text-gray-300 text-sm">Posted on
                                        {{ $post->created_at?->format('F d, Y - h:i a') }} <br> @if($post->created_at !== $post->updated_at)(Edited)@endif</p>
                                </div>
                            </div>
                ...
                <div id="image-preview" class="flex flex-wrap mt-4">
                    @foreach($post->communityPostAttachments as $upload)
                        <input type="hidden" class="uploaded-file-{{$upload->community_post_id}}" data-name="{{ $upload->original_name }}" value="{{ asset("storage/{$upload->path}") }}">
                    @endforeach
                </div>
            @endforeach
        @endif
            ...
            function editPost(element) {
                ...
                new Dropzone('#editDropzone', {
                    ...
                    init: function() {
                        ...
                        document.querySelectorAll(`.uploaded-file-${$(element).data('id')}`).forEach(function (input) {
                            let mockFile = { name: $(input).data('name') }; // Example file properties
